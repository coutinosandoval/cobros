...
--%> 
   ...
   <input type="text" name="fechacompras" hidden="true"
    ...
    Precio de Compra: <input type="text" name="txtpreciocompra" maxlength="10" required="true" id="txtpreciocompra" value="" />
     Precio de Venta <input type="text" name="txtprecioventa" maxlength="10" required="true" id="txtpreciocompra" value="" />
<%-- 
SE ANULO ESTE CAMPO PORQUE EL CLIENTE DESEABA APLICAR UN % DE VENTA, PERO  QUEDA HABILITADO
EN EL MOMENTO QUE ASI SE DESEE

Precio Venta:<input type="text" name="txtprecioventa" value="0" maxlength="10" size="10" />
   --%>  
...
{ 
 ...
 //  out.println("<td>"+tablatemporal.get(i).getfvencimiento()+"</td>  ");
   out.println("<td align=right >"+(format.format(tablatemporal.get(i). getprecio()))+"</td>");
   out.println("<td align=right>"+(format.format(tablatemporal.get(i).getprecioventa()))+"</td>");
   out.println("<td>" + "<a href=comprasborraproductosdefactura.jsp?codigo&factura="+
       tablatemporal.get(i).getcodigo() +">"
       +"Eliminar"+"</a>"+"</td>");
   out.println("</tr>");
